...
namespace Risk_assessment_by_criteria
{
    public partial class EditComp : Form
    {
        ...
        {
            //List<string> threats = new List<string>(); // заполнить список угроз (сейчас пустой) 
            if (Component.changeComp(textBox_name.Text.ToString(), comboBox_area.SelectedItem.ToString()))
            {
                Component.isSuccEd = true;
                MessageBox.Show("Компонент успешно изменен", "Изменение компонента", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
}
