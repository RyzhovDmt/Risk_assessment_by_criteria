...
{
    public partial class NewThreat : Form
    {
        ...
        private void button_add_Click(object sender, EventArgs e)
        {
            ...
            if (Threat.addToFile(textBox_name.Text, textBox_weakness.Text, float.Parse(textBox_v.Text), Convert.ToInt32(textBox_s.Text), comp_list))
            {
                Threat.isSuccAdd = true;
                MessageBox.Show("Угроза успешно добавлена", "Новая угроза", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                Threat.isSuccAdd = false;
                MessageBox.Show("Угроза с таким именем уже существует", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            //thr.add_to_list(textBox_name.Text, textBox_weakness.Text, float.Parse(textBox_v.Text), Convert.ToInt32(textBox_s.Text), comp_list);
        }
        ...
        private void buttonCVSS_Click(object sender, EventArgs e)
        {
            CVSS f1 = new CVSS();
            f1.Owner = this;
            f1.ShowDialog();
